...
def _is_zero(val) -> bool:
    ...
    try:
        ...
        try: return abs(float(_normalize_decimal(val))) < 1e-12
        except: return False
...
class BinanceReportParser:
    def __init__(self, file_path: str):
        ...
        self.identifiers = ExtractedIdentifiers(source_file=self.file_path.name, exchange="binance")
    ...
    def parse_all(self) -> ExtractedIdentifiers:
        ...
        for sheet_name in all_sheets:
            try: self._parse_sheet(sheet_name)
            except Exception as e: print(f"  [!] Błąd w arkuszu '{sheet_name}': {e}")
        ...
        if foreign_user_ids:
            ...
            for uid in foreign_user_ids: self.identifiers.related_user_ids.add(uid)
        elif all_fiat:
    ...
    def _dedup_phones(self):
        ...
        for p1 in phones:
            for p2 in phones:
                if p1 == p2: continue
                c1 = p1.lstrip("+").replace(" ", "").replace("-", "")
                c2 = p2.lstrip("+").replace(" ", "").replace("-", "")
                if len(c1) > len(c2) and c1.endswith(c2): to_remove.add(p2)
                elif len(c2) > len(c1) and c2.endswith(c1): to_remove.add(p1)
    ...
    def _parse_sheet(self, sheet_name: str):
        if sheet_name == "Customer Information":
            ...
            if internal_name:
                method = getattr(self, f"_parse_{internal_name}", self._parse_generic)
    ...
    def _parse_assets_overview(self, df: pd.DataFrame):
                        ...
                        if ni < len(df) and nj < len(df.columns):
                            ...
                            if btc_val and not _is_zero(btc_val):
                                ...
                                for token in tokens:
                                    ...
                                    if m:
                                        num = m.group()
                                        if not btc_str: btc_str = _fmt_num(num)
                                        elif not usd_str: usd_str = _fmt_num(num); break
                                if not btc_str: btc_str = _fmt_num(btc_val)
                                self.identifiers.estimate_total_btc = btc_str
                                if usd_str: self.identifiers.estimate_total_usdt = usd_str
        ...
        while i < len(df):
            ...
            if "spot" in row_str: current_wallet = "Spot"
            elif "futures" in row_str: current_wallet = "Futures"
            elif "earn" in row_str: current_wallet = "Earn"
            elif "margin" in row_str: current_wallet = "Margin"
            elif "pool" in row_str: current_wallet = "Pool"
            elif "funding" in row_str: current_wallet = "Funding"
            if "currency name" in row_str or "currency code" in row_str or "all positions" in row_str:
                ...
                for idx_h, h in enumerate(headers):
                    if h:
                        ...
                        if "currency name" in h_lower: col_map["currency_name"] = idx_h
                        elif "currency code" in h_lower or (h_lower == "code" and "currency" not in row_str): col_map["currency_code"] = idx_h
                        elif "all positions" in h_lower or h_lower == "all": col_map["all_positions"] = idx_h
                        elif "available" in h_lower: col_map["available_positions"] = idx_h
                        elif "withdrawal" in h_lower or "in withdrawal" in h_lower: col_map["in_withdrawal"] = idx_h
                        elif "pending" in h_lower: col_map["pending_order"] = idx_h
                        elif "btc equivalent" in h_lower or ("btc" in h_lower and "equivalent" in h_lower): col_map["btc_equivalent"] = idx_h
                        elif "usdt equivalent" in h_lower or ("usdt" in h_lower and "equivalent" in h_lower): col_map["usdt_equivalent"] = idx_h
    ...
    def _parse_spot_asset_log(self, df: pd.DataFrame, sheet_name: str): self._parse_asset_log(df, "Spot", sheet_name)
    def _parse_funding_asset_log(self, df: pd.DataFrame, sheet_name: str): self._parse_asset_log(df, "Funding", sheet_name)

    def _parse_asset_log(self, df: pd.DataFrame, wallet_type: str, sheet_name: str):
        cols = [str(c).strip().lower() for c in df.columns]
        col_map = {}
        for idx, c in enumerate(cols):
            if "time" in c or "date" in c: col_map["time"] = idx
            elif c == "currency" or c == "asset" or "currency" in c: col_map["currency"] = idx
            elif "change" in c: col_map["change"] = idx
            elif "amount" in c: col_map["amount"] = idx
            elif "locked" in c: col_map["locked"] = idx
            elif "freeze" in c: col_map["freeze"] = idx
            elif "processing" in c: col_map["processing"] = idx
            elif "reason" in c or "type" in c or "operation" in c: col_map["reason"] = idx
            elif "description" in c or "desc" in c or "note" in c: col_map["description"] = idx
            elif "available" in c: col_map["available"] = idx
            elif "transaction id" in c or "txid" in c or "tx id" in c: col_map["transaction_id"] = idx
            elif "user id" in c or "userid" in c: col_map["user_id"] = idx
            elif "order id" in c or "orderid" in c or "commission id" in c: col_map["order_id"] = idx
        ...
        for txid, rows in groups.items():
            for r in rows:
                ...
                if chg_val is None: chg_val = r.get("amount")
                reason_parts = []
                if r.get("reason"): reason_parts.append(str(r["reason"]))
                if r.get("description"): reason_parts.append(str(r["description"]))
                ...
                if txn.time or txn.currency:
                    if wallet_type == "Spot": self.identifiers.spot_transactions.append(txn)
                    else: self.identifiers.funding_transactions.append(txn)
                if r.get("user_id"): self._add_related_user_id(r["user_id"])
                if r.get("order_id"):
                    v = clean_val(r["order_id"])
                    if v: self.identifiers.order_ids.add(str(v))
        ...
        if "Transaction ID" in df.columns:
            for v in df["Transaction ID"].dropna():
                v = clean_val(v)
                if v: self.identifiers.transaction_ids.add(v)
        ...
        while i < df.shape[0]:
            ...
            if current_section and headers is not None and len(non_null) >= 1:
                ...
                for h, v in zip(headers, values):
                    if h is not None: sections[current_section][h] = v if v is not None else ""
                headers = None
                i += 1; continue
            i += 1
                ...
                for col, val in data.items():
                    ...
                    if col_lower in ["user id", "userid", "user_id", "uid", "binance id", "binanceid"]:
                        ...
                        if v_norm:
                            ...
                            print(f"  Wykryto ID właściciela: {v_norm}")
        for section_name, data in sections.items():
            for col, val in data.items():
                if val is None or val == "": continue
                v = str(val).strip()
                email = extract_email(v)
                if email: self.identifiers.emails.add(email); continue
                phone = extract_phone(v)
                if phone: self.identifiers.phones.add(phone); continue
                if is_valid_ip(v): self.identifiers.ips.add(v); continue
                if re.match(r"^[A-Z]{1,2}\d{6,10}$", v): self.identifiers.id_numbers.add(v); continue
                if col and "nationality" in str(col).lower() and len(v) == 2 and v.isalpha(): self.identifiers.nationalities.add(v); continue

    def _format_card_number(self, card_bin: str, card_last4: str) -> Optional[str]:
        if not card_bin or not card_last4: return None
        bin_clean = str(card_bin).strip().replace(" ", "")
        last4_clean = str(card_last4).strip().replace(" ", "")
        if not bin_clean.isdigit() or not last4_clean.isdigit(): return None
        if len(bin_clean) >= 6: return f"{bin_clean[:4]} {bin_clean[4:6]}** **** {last4_clean}"
        elif len(bin_clean) >= 4: return f"{bin_clean[:4]} **** **** {last4_clean}"
        return f"**** **** **** {last4_clean}"

    def _add_related_user_id(self, val):
        v_norm = self._normalize_user_id(val)
        if v_norm and v_norm not in self.identifiers.user_ids: self.identifiers.related_user_ids.add(v_norm)
    ...
    def _normalize_user_id(self, val) -> Optional[str]:
        ...
        v = v.replace(" ", "").replace(",", "")
        if re.match(r"^\d{6,12}$", v): return v
        return None
        ...
        try:
            ...
            if images:
                ...
                for idx, img in enumerate(images):
                    ...
                    with open(img_path, "wb") as fimg: fimg.write(img._data())
                    self.identifiers.kyc_images.append(str(img_path))
                    print(f"  Zapisano obrazek KYC: {img_path.name}")
            else: print(f"  Brak osadzonych obrazków w KYC Documents")
        except Exception as e: print(f"  [!] Błąd przy wyciąganiu obrazków KYC: {e}")

    def _parse_access_logs(self, df: pd.DataFrame, sheet_name: str):
        for col, target in [("Real IP", self.identifiers.ips), ("Geolocation", self.identifiers.geolocations), ("Browser", self.identifiers.browsers)]:
            if col in df.columns:
                for v in df[col].dropna():
                    v = clean_val(v)
                    if v:
                        if col == "Real IP" and is_valid_ip(v): target.add(v)
                        elif col != "Real IP": target.add(v)
        if "User ID" in df.columns:
            for v in df["User ID"].dropna():
                v = clean_val(v)
                if v: self._add_related_user_id(v)

    def _parse_approved_devices(self, df: pd.DataFrame, sheet_name: str):
        if "IP Address" in df.columns:
            for v in df["IP Address"].dropna():
                v = clean_val(v)
                if v and is_valid_ip(v): self.identifiers.ips.add(v)
        if "Geolocation" in df.columns:
            for v in df["Geolocation"].dropna():
                v = clean_val(v)
                if v: self.identifiers.geolocations.add(v)
        if "Key" in df.columns and "Value" in df.columns:
            for _, row in df.iterrows():
                ...
                key_lower = key.lower()
                if "fvideo_id" in key_lower: self.identifiers.fvideo_ids.add(val)
                elif "bnc-uuid" in key_lower or "bnc_uuid" in key_lower: self.identifiers.bnc_uuids.add(val)
                elif "device_id" in key_lower: self.identifiers.device_ids.add(val)
                elif "login_ip" in key_lower and is_valid_ip(val): self.identifiers.ips.add(val)

    def _parse_deposit_history(self, df: pd.DataFrame, sheet_name: str):
        cols = [str(c).strip().lower() for c in df.columns]
        col_map = {}
        for idx, c in enumerate(cols):
            if "time" in c or "date" in c or "create" in c: col_map["time"] = idx
            elif "currency" in c: col_map["currency"] = idx
            elif "amount" in c: col_map["amount"] = idx
            elif "status" in c: col_map["status"] = idx
            elif "txid" in c or "transaction id" in c: col_map["txid"] = idx
            elif "deposit address" in c: col_map["deposit_address"] = idx
            elif "source address" in c: col_map["source_address"] = idx
            elif "counterparty" in c or "counter party" in c: col_map["counterparty"] = idx
            elif "user id" in c: col_map["user_id"] = idx
        ...
        for _, row in df.iterrows():
            ...
            if curr and amt:
                txn = AssetTransaction(time=time_str, currency=curr, amount=_fmt_num(amt), change=_fmt_num(amt), reason=f"Deposit ({status})" if status else "Deposit", transaction_id=txid, wallet_type="Deposit", source_sheet=sheet_name)
            ...
            if "deposit_address" in col_map:
                ...
                if v and is_wallet_address(v): self.identifiers.wallet_addresses.add(v)
            if "source_address" in col_map:
                v = clean_val(row.iloc[col_map["source_address"]]); 
                if v and is_wallet_address(v): self.identifiers.wallet_addresses.add(v)
            if "txid" in col_map:
                v = clean_val(row.iloc[col_map["txid"]]); 
                if v and is_txid(v): self.identifiers.txids.add(v)
            ...
            if "counterparty" in col_map:
                ...
                if v: self.identifiers.counterparty_ids.add(str(v))
    ...
    def _parse_withdrawal_history(self, df: pd.DataFrame, sheet_name: str):
        ...
        col_map = {}
        for idx, c in enumerate(cols):
            if "time" in c or "date" in c or "apply" in c: col_map["time"] = idx
            elif "currency" in c: col_map["currency"] = idx
            elif "amount" in c: col_map["amount"] = idx
            elif "status" in c: col_map["status"] = idx
            elif "txid" in c or "transaction id" in c: col_map["txid"] = idx
            elif "destination address" in c: col_map["dest_address"] = idx
            elif "counterparty" in c or "counter party" in c: col_map["counterparty"] = idx
            elif "user id" in c: col_map["user_id"] = idx
        ...
        for _, row in df.iterrows():
            ...
            txid = str(clean_val(row.iloc[col_map.get("txid", 0)])) if "txid" in col_map else ""
            if curr and amt:
                txn = AssetTransaction(time=time_str, currency=curr, amount=_fmt_num(amt), change=_fmt_num(f"-{amt}"), reason=f"Withdrawal ({status})" if status else "Withdrawal", transaction_id=txid, wallet_type="Withdrawal", source_sheet=sheet_name)
    ...
    def _parse_fiat_deposit(self, df: pd.DataFrame, sheet_name: str):
        ...
        for col, target in [("Card Bin", self.identifiers.card_bins), ("Card Last 4 Digital", self.identifiers.card_last4), ("Iban", self.identifiers.ibans), ("Account Number", self.identifiers.account_numbers)]:
            if col in df.columns:
                for v in df[col].dropna():
                    v = clean_val(v)
                    if v: target.add(v)
        ...
        for _, row in df.iterrows():
            ...
            if status == "fail" or "fail" in status: fail_count += 1; continue
            elif status in ["success", "completed", "filled", "confirmed"]: success_count += 1
            ...
            if not crypto_amt: crypto_amt = clean_val(row.get("Crypto Amount")) if "Crypto Amount" in df.columns else None
            ...
            if fiat_curr and fiat_amt and not _is_zero(fiat_amt):
                self.identifiers.fiat_deposit_transactions.append(AssetTransaction(time=time_str, currency=fiat_curr, amount=_fmt_num(fiat_amt), change=_fmt_num(f"-{fiat_amt}"), reason=f"Płatność {fiat_curr} za zakup {crypto_curr or 'krypto'} | Status: {status_raw or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
            if crypto_curr and crypto_amt and not _is_zero(crypto_amt):
                self.identifiers.fiat_deposit_transactions.append(AssetTransaction(time=time_str, currency=crypto_curr, amount=_fmt_num(crypto_amt), change=_fmt_num(crypto_amt), reason=f"Zakup {crypto_curr} za {fiat_curr or 'fiat'} | Status: {status_raw or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
        if fail_count > 0: print(f"  ⚠️  Fiat Deposit: {fail_count} transakcji FAIL")
        if success_count > 0: print(f"  ✓ Fiat Deposit: {success_count} transakcji Success")
    ...
    def _parse_fiat_trades(self, df: pd.DataFrame, sheet_name: str):
        ...
        if "Order Id" in df.columns:
            for v in df["Order Id"].dropna():
                v = clean_val(v)
                if v: self.identifiers.order_ids.add(v)
        if "User Id" in df.columns:
            for v in df["User Id"].dropna():
                v = clean_val(v)
                if v: self._add_related_user_id(v)
        for _, row in df.iterrows():
            ...
            if status and status not in ["completed", "success", "filled", "confirmed", ""]:
                if any(x in status for x in ["fail", "error", "cancel", "reject", "expired"]): continue
            time_str = str(clean_val(row.get("Order Create Time"))) if "Order Create Time" in df.columns else ""
            ...
            if is_buy:
                if fiat_curr and fiat_amt and not _is_zero(fiat_amt):
                    self.identifiers.fiat_trade_transactions.append(AssetTransaction(time=time_str, currency=fiat_curr, amount=_fmt_num(fiat_amt), change=_fmt_num(f"-{fiat_amt}"), reason=f"Płatność {fiat_curr} za zakup {crypto_curr or 'krypto'} | Status: {status or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
                if crypto_curr and crypto_amt and not _is_zero(crypto_amt):
                    self.identifiers.fiat_trade_transactions.append(AssetTransaction(time=time_str, currency=crypto_curr, amount=_fmt_num(crypto_amt), change=_fmt_num(crypto_amt), reason=f"Zakup {crypto_curr} za {fiat_curr or 'fiat'} | Status: {status or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
            elif is_sell:
                if fiat_curr and fiat_amt and not _is_zero(fiat_amt):
                    self.identifiers.fiat_trade_transactions.append(AssetTransaction(time=time_str, currency=fiat_curr, amount=_fmt_num(fiat_amt), change=_fmt_num(fiat_amt), reason=f"Otrzymano {fiat_curr} ze sprzedaży {crypto_curr or 'krypto'} | Status: {status or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
                if crypto_curr and crypto_amt and not _is_zero(crypto_amt):
                    self.identifiers.fiat_trade_transactions.append(AssetTransaction(time=time_str, currency=crypto_curr, amount=_fmt_num(crypto_amt), change=_fmt_num(f"-{crypto_amt}"), reason=f"Sprzedaż {crypto_curr} na {fiat_curr or 'fiat'} | Status: {status or 'N/A'}", transaction_id=order_id, wallet_type="Fiat", source_sheet=sheet_name, user_id=user_id))
    ...
    def _parse_binance_pay(self, df: pd.DataFrame, sheet_name: str):
        if "Counterparty Wallet ID" in df.columns:
            for v in df["Counterparty Wallet ID"].dropna():
                v = clean_val(v)
                if v: self.identifiers.counterparty_ids.add(v)
    ...
    def _parse_otc_trading(self, df: pd.DataFrame, sheet_name: str):
        ...
        col_map = {}
        for idx, c in enumerate(cols):
            if "order" in c and "id" in c: col_map["order_id"] = idx
            elif "user" in c and "id" in c: col_map["user_id"] = idx
            elif "side" in c or "type" in c: col_map["side"] = idx
            elif "status" in c: col_map["status"] = idx
            elif "symbol" in c or "pair" in c: col_map["symbol"] = idx
            elif "base" in c and "asset" in c: col_map["base_asset"] = idx
            elif "quote" in c and "asset" in c: col_map["quote_asset"] = idx
            elif "base" in c and "qty" in c: col_map["base_qty"] = idx
            elif "quote" in c and "qty" in c: col_map["quote_qty"] = idx
            elif "time" in c or "date" in c: col_map["time"] = idx
        ...
        if "base_asset" in col_map and "base_qty" in col_map:
            for _, row in df.iterrows():
                ...
                if not base or not base_qty: continue
                base_chg = f"-{base_qty}" if side == "SELL" else (f"+{base_qty}" if side == "BUY" else base_qty)
                self.identifiers.spot_transactions.append(AssetTransaction(time=time_str, currency=base, amount=_fmt_num(base_qty), change=_fmt_num(base_chg), reason=f"OTC {side} {base}{quote} ({status})", wallet_type="OTC", source_sheet=sheet_name))
                if quote and quote_qty:
                    quote_chg = f"+{quote_qty}" if side == "SELL" else (f"-{quote_qty}" if side == "BUY" else quote_qty)
                    self.identifiers.spot_transactions.append(AssetTransaction(time=time_str, currency=quote, amount=_fmt_num(quote_qty), change=_fmt_num(quote_chg), reason=f"OTC {side} {base}{quote} ({status})", wallet_type="OTC", source_sheet=sheet_name))
            print(f"  Sparsowano OTC")
    ...
    def _parse_generic(self, df: pd.DataFrame, sheet_name: str):
        ...
        for col in df.columns:
            for v in df[col].dropna():
                v = clean_val(v)
                if not v: continue
                if is_valid_ip(v): self.identifiers.ips.add(v); found["ips"] += 1
                elif is_wallet_address(v): self.identifiers.wallet_addresses.add(v); found["wallets"] += 1
                elif is_txid(v): self.identifiers.txids.add(v); found["txids"] += 1
                else:
                    email = extract_email(v)
                    if email: self.identifiers.emails.add(email); found["emails"] += 1
        if sum(found.values()) > 0: print(f"  Generycznie znaleziono: {found}")
