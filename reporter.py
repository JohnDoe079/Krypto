...
# Szerokość strony A4 minus marginesy (~15,5 cm = 6,1 cala)
...
class ReportGenerator:
    ...
    def _sort_basic_info(self, data: Dict[str, str]) -> List[List[str]]:
        """Sortuje wiersze Basic Information zgodnie z BASIC_INFO_ORDER."""
        ...
        rows = [[col, val if val is not None and str(val).strip() != "" else "(puste)"] for _, col, val in priority]
        rows += [[col, val if val is not None and str(val).strip() != "" else "(puste)"] for col, val in rest]
        return rows
    ...
    def generate(self, reports: List[ExtractedIdentifiers], file_map: Dict[str, str]):
        ...
        self._add_table(
            ["Plik", "Giełda", "Arkusze", "ID użytkowników", "E-maile", "IP", "Portfele", "TXID"],
            summary_rows
        )
        ...
        for idx, r in enumerate(reports, 1):
            ...
            self._add_paragraph(f"Przeanalizowane arkusze ({len(r.parsed_sheets)}): {', '.join(r.parsed_sheets)}")
            if r.unknown_sheets:
                self._add_paragraph(f"Nieznane arkusze: {', '.join(r.unknown_sheets)}", color=RGBColor(0xC0, 0x00, 0x00))
            ...
            if "KYC Documents" in r.parsed_sheets:
                ...
                if r.kyc_images:
                    ...
                    for idx, img_path in enumerate(r.kyc_images):
                        row_idx = idx // n_cols
                        col_idx = idx % n_cols
                    ...
                    self._add_paragraph("(W niektórych plikach Binance zdjęcia dokumentów mogą być osadzone w tym arkuszu.)")
        ...
        if len(reports) >= 2:
            ...
            if common:
                ...
                for field_name, values in common.items():
                    self._add_heading(f"[{field_name}] — {len(values)} wspólnych", level=3)
                    rows = [[str(i+1), str(v)] for i, v in enumerate(values)]
                    self._add_table(["Lp.", "Wartość"], rows, max_rows=30)
            else:
